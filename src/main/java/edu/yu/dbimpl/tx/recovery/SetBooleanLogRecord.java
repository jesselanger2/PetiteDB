...
/**
 * A SETBOOLEAN log record logs an update to a boolean value.
 * Contains the old value needed for undo.
 */
public class SetBooleanLogRecord implements LogRecord {
    ...
    public SetBooleanLogRecord(int txnum, BlockIdBase blk, int offset, boolean oldValue) {
        this.txnum = txnum;
        this.blk = blk;
        this.offset = offset;
    ...
    public void undo(BufferBase buff) {
        ...
        page.setBoolean(offset, oldValue);
    }

    @Override
    public byte[] toBytes() {
        byte[] filenameBytes = blk.fileName().getBytes(StandardCharsets.UTF_8);
        int size = Integer.BYTES * 5 + filenameBytes.length + 1;

        ByteBuffer bb = ByteBuffer.allocate(size);
        bb.putInt(SETBOOLEAN);
        bb.putInt(txnum);
        bb.putInt(filenameBytes.length);
        bb.put(filenameBytes);
        bb.putInt(blk.number());
        bb.putInt(offset);
        bb.put((byte) (oldValue ? 1 : 0));

        return bb.array();
    }

    static SetBooleanLogRecord fromBytes(ByteBuffer bb) {
        int txnum = bb.getInt();
        ...
        boolean oldValue = bb.get() != 0;

        BlockIdBase blk = new BlockId(filename, blknum);
        return new SetBooleanLogRecord(txnum, blk, offset, oldValue);
    }
    ...
    public String toString() {
        return "SETBOOLEAN <" + txnum + ", " + blk + ", " + offset + ", " + oldValue + ">";
    }
}