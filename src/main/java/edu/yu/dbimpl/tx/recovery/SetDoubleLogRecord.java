...
/**
 * A SETDOUBLE log record logs an update to a double value.
 * Contains the old value needed for undo.
 */
public class SetDoubleLogRecord implements LogRecord {
    ...
    public SetDoubleLogRecord(int txnum, BlockIdBase blk, int offset, double oldValue) {
        this.txnum = txnum;
        this.blk = blk;
        this.offset = offset;
    ...
    public void undo(BufferBase buff) {
        ...
        page.setDouble(offset, oldValue);
    }

    @Override
    public byte[] toBytes() {
        byte[] filenameBytes = blk.fileName().getBytes(StandardCharsets.UTF_8);
        int size = Integer.BYTES * 5 + filenameBytes.length + Double.BYTES;
        ...
        bb.putInt(offset);
        bb.putDouble(oldValue);

        return bb.array();
    }

    static SetDoubleLogRecord fromBytes(ByteBuffer bb) {
        int txnum = bb.getInt();
        ...
        double oldValue = bb.getDouble();

        BlockIdBase blk = new BlockId(filename, blknum);
        return new SetDoubleLogRecord(txnum, blk, offset, oldValue);
    }
    ...
    public String toString() {
        return "SETDOUBLE <" + txnum + ", " + blk + ", " + offset + ", " + oldValue + ">";
    }
}